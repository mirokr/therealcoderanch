/* The A-Team, 27/04/2021 */
/* 
Form and datepicker code: line 9-82
Lazy video code:          line 84-102
Toast customized code: 	  line 309-326
Datepicker source code:   line 327-2689 (just changed import location for arrow icons)
Toast source code:        line 103-307*/
/* datepicker */
$(document).ready(function(){
		$( "#datepicker").datepicker({ dateFormat: 'dd-mm-yy', onSelect: showDays});
		} );
$(document).ready(function(){
		$( "#datepicker2").datepicker({ dateFormat: 'dd-mm-yy', onSelect: showDays });
		} );
		var days=0;
function showDays() {
...
$(document).ready(function(){
	$("select.package").change(function(){
		...
		if(selectedPackage==1){discount2=1.1;}
	});
	
	$("select.adults").change(function(){  
		adults=0;
		var selectedAdults = $(this).children("option:selected").val();
		if(selectedAdults==1){adults=1;}
		else if(selectedAdults==0){adults=0;}
		else if(selectedAdults==2){adults=2;}
		else if(selectedAdults==3){adults=3;}
		else if(selectedAdults==4){adults=4;}
	});
	
	$("select.children").change(function(){
		var selectedChildren = $(this).children("option:selected").val();
		if(selectedChildren==1){children=1;}
		else if(selectedChildren==0){children=0;}
		else if(selectedChildren==2){children=2;}
		else if(selectedChildren==3){children=3;}
		else if(selectedChildren==4){children=4;}
		else if(selectedChildren==5){children=5;}
		showToast1() ;
	});
	
		$("select.bungalow").change(function(){
		discount=1;	
		var selectedBungalow = $(this).children("option:selected").val();
		if(selectedBungalow==3||selectedBungalow==4){discount=1.1;}
	});
});
...
function totMethod(){
	...
	var tot=0;
if(adults!=0&&days!=0){
		tot=days*adults*price;
}else{total=0;}

if(children!=0&&adults!=0){
	childCost=children*price*0.5*days;		
	}
else{childCost=0;}
	total=((tot+childCost)*discount*discount2).toFixed(0);		
	$('#totalPrice').val(total+" euros"); 
	event.preventDefault();	
	showToast4();
}
// lazy youtube video on fun page(2 second delay)
...
function showToast1() 
	{
		var message = ("Please note 50% discount on children!");		
		$("body").cftoaster({content: message,animationTime: 300,fontColor: "white",
		bottomMargin: 400 });	
	}
function showToast3() 
	{
		var message = ("For Teams or large groups over 10 please contact us for a 20% discount.");		
		$("body").cftoaster({content: message,animationTime: 300,fontColor: "white",
		bottomMargin: 300 });		
	}		
function showToast4() 
	{
		var message = ("Please note, this is just a college project concept!             If its you Elon, please use contact link below for a great investment oportunity!");		
		$("body").cftoaster({content: message,animationTime: 1000,fontColor: "red",
		bottomMargin: 400, showTime: 6000, });
	}	
/*!
